...
const theatreData = [
  ...
  { 
    id: 6, name: "Luxe Cinemas Phoenix", 
...
function Theatres() {
  const [theatres, setTheatres] = useState(
    theatreData.map(t => ({ ...t, liked: false }))
  );

function likeHandler(id) {
  const updated = theatres.map(function (t) {
    if (t.id === id) {
      ...
      {theatres.map(theatre => (
        <div className="theatre-card" key={theatre.id}>
          <div className="theatre-header">
            ...
            <FaHeart onClick={() => likeHandler(theatre.id)} color={theatre.liked ? "red" : "#aaa"}
              style={{ cursor: "pointer" }} size={20}/>
          </div>

          <div className="showtime-list">
            {theatre.times.map((time, i) => (
              <button key={i}> {time}<br /><span>3D</span></button>))}
          </div>
        </div>
      ))}
    </div>
